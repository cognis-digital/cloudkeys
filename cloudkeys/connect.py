"""Native cognis-connect emit for cloudkeys — forward findings to any platform.

Maps cloudkeys's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cloudkeys ... --format json | cloudkeys-emit --to stix
    cloudkeys-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('key', '') + ' ' + rec.get('service', '')
        out['severity'] = rec.get('severity', 'low')
        out['type'] = 'leaked credential'
        out['description'] = f"Leaked {rec.get('service', '')} credential"
        out['tags'] = [rec.get('service', ''), 'credential']
